...
def discover_vllm_url(service_name="gemma4-vllm-gpu"):
    if os.getenv("VLLM_BASE_URL"):
        return os.getenv("VLLM_BASE_URL")

    # Try Azure Container App
    import subprocess
    import json
    try:
        cmd = [
        ...
        if res.returncode == 0:
            apps = json.loads(res.stdout.strip())
            if apps:
                for app in apps:
                    fqdn = app.get("fqdn")
                    if fqdn:
        ...
        print(f"Error discovering Azure Container App URL: {e}")
    return None


# Raise file descriptor limit for high concurrency benchmark client
try:
    soft, hard = resource.getrlimit(resource.RLIMIT_NOFILE)
    resource.setrlimit(resource.RLIMIT_NOFILE, (max(4096, soft), hard))
    print(f"Raised open files limit from {soft} to {max(4096, soft)}")
except Exception as e:
    print(f"Warning: Could not raise open files limit: {e}")
...
async def run_sweep():
    ...
    # Sweep dimensions requested: 4, 8, 16..16K context window and 1, 2..1024 concurrent users
    ...
    with open(md_file, "w") as f:
        ...
        f.write(f"Model: `{model_name}` (NVIDIA A100 GPU Azure Container Apps NC24-A100)\n\n")

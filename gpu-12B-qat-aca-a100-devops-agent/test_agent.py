...
class TestDevOpsAgent(unittest.IsolatedAsyncioTestCase):
    ...
    def test_update_vllm_scaling(self, mock_run):
        """Test the update_vllm_scaling tool with mock Azure Container App show and update."""
        from server import update_vllm_scaling

        # Mock Container App show
        mock_show_res = MagicMock()
        mock_show_res.returncode = 0
        mock_show_res.stdout = json.dumps({"properties": {"provisioningState": "Succeeded"}})

        # Mock Container App update
        mock_update_res = MagicMock()
        mock_update_res.returncode = 0

        mock_run.side_effect = [mock_show_res, mock_update_res]

        result = update_vllm_scaling(instance_type="gpu-profile-new", service_name="test-service")

        # Verify calls
        self.assertEqual(mock_run.call_count, 2)
        args_show, kwargs_show = mock_run.call_args_list[0]
        self.assertIn("containerapp", args_show[0])
        self.assertIn("show", args_show[0])
        args_update, kwargs_update = mock_run.call_args_list[1]
        self.assertIn("update", args_update[0])
        self.assertIn("gpu-profile-new", args_update[0])

        self.assertIn(
            "Successfully requested update of Azure Container App workload profile to `gpu-profile-new`.",
            result,
        )
    ...
    async def test_deploy_vllm(self, mock_get_secret, mock_run):
        """Test the deploy_vllm tool with mock Azure CLI Container App commands."""
        from server import deploy_vllm

        mock_get_secret.return_value = "mock-hf-token"

        # Mock responses: 1. RG create, 2. Env create, 3. Workload Profile add, 4. Container App create, 5. Get FQDN
        mock_rg_res = MagicMock()
        mock_rg_res.returncode = 0

        mock_env_res = MagicMock()
        mock_env_res.returncode = 0

        mock_wp_res = MagicMock()
        mock_wp_res.returncode = 0

        mock_app_res = MagicMock()
        mock_app_res.returncode = 0

        mock_fqdn_res = MagicMock()
        mock_fqdn_res.returncode = 0
        mock_fqdn_res.stdout = "test-service.eastus.azurecontainerapps.io\n"

        mock_run.side_effect = [mock_rg_res, mock_env_res, mock_wp_res, mock_app_res, mock_fqdn_res]
        ...
        # Inspect the container app create call
        app_create_args = mock_run.call_args_list[3][0][0]
        self.assertIn("containerapp", app_create_args)
        self.assertIn("create", app_create_args)
        self.assertIn("gpu-profile", app_create_args)

    @patch("subprocess.run")
    async def test_destroy_vllm(self, mock_run):
        """Test the destroy_vllm tool with mock Azure CLI container app show and delete."""
        from server import destroy_vllm

        # Mock containerapp show exists, and containerapp delete succeeds
        mock_show_res = MagicMock()
        mock_show_res.returncode = 0
        mock_show_res.stdout = "{}"

        mock_del_res = MagicMock()
        mock_del_res.returncode = 0

        mock_run.side_effect = [mock_show_res, mock_del_res]
    ...
    def test_status_vllm(self, mock_run):
        """Test status_vllm tool with mock Azure CLI Container App show details."""
        ...
        self.assertIn("Azure Container App Status for service 'test-service'", result)
        self.assertIn("test-service-app", result)
        self.assertIn("Succeeded", result)
        self.assertIn("Running", result)
        self.assertIn("https://test-service.eastus.azurecontainerapps.io", result)
    ...
    @patch("subprocess.run")
    async def test_save_hf_token(self, mock_run):
        """Test save_hf_token tool saves token to Azure Key Vault."""
        ...
        self.assertIn("Token saved to Azure Key Vault", result)
    ...
    async def test_get_help(self):
        ...
        self.assertIn("Azure Gemma 4 SRE Agent Help", result)

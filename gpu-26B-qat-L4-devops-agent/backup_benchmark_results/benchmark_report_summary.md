...
This report presents performance benchmark results for the self-hosted **Gemma 4 26B QAT (Quantization-Aware Training)** model (`google/gemma-4-26B-A4B-it-qat-w4a16-ct`) deployed on a single **NVIDIA L4 GPU** (Cloud Run Gen2) in the `us-east4` region.
...
This chart compares the serving characteristics of the Standard 26B model (using FP8 quantization) and the QAT 26B model (INT4 quantization).
...
* **VRAM Capacity Boost**: The 26B Standard (bfloat16) model leaves 0 GB of free VRAM for the KV cache on a single L4 GPU, causing stability issues at concurrencies above 8.
* **The QAT Advantage**: The 26B QAT (w4a16) model frees up **~18 GB of VRAM** for the KV cache, permitting **100% success rate up to 512 concurrent users** (a ~64x improvement in concurrency capacity).

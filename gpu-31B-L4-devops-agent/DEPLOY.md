# Deployment Guide: Self-Hosted vLLM on Cloud Run (Gemma 4 31B-it)

This document summarizes the deployment state and configuration for the vLLM inference server used by the DevOps Agent.

## 📦 Model Artifacts
The model was extracted from `gemma-4-31B-it.tar.gz` and uploaded to Google Cloud Storage.
...
*   **Service Name:** `gpu-31b-l4-devops-agent`
...
```bash
export VLLM_BASE_URL="https://gpu-31b-l4-devops-agent-289270257791.us-east4.run.app"
...
```bash
gcloud beta run deploy gpu-31b-l4-devops-agent \

...
def discover_vllm_url(service_name: str = DEFAULT_SERVICE_NAME) -> Optional[str]:
    """Attempts to automatically discover the Azure Container App URL, VM public IP, or Cloud Run service URL."""
    if VLLM_BASE_URL:
...
async def deploy_vllm(
    ...
    """
    Deploys vLLM to Azure Container Apps (ACA) with GPU (Consumption-GPU-NC8as-T4).
...
async def start_azure_vm(
    ...
    key_name: str = "unused",
    subnet_id: Optional[str] = None,
    instance_type: str = "Consumption-GPU-NC8as-T4",
    market_type: str = "on-demand",
    instance_id: Optional[str] = None,
) -> str:
    """
    Starts an existing stopped Azure Container App or VM, or provisions a new Container App with T4 GPU if none exists.
    ...
    Args:
        ...
        instance_type: GPU workload profile (default: 'Consumption-GPU-NC8as-T4').
...
def get_vllm_gpu_deployment_config(
    ...
    """
    Generates Azure Container Apps (ACA) CLI deployment commands for GPU (NVIDIA T4).
...
To deploy vLLM on Azure Container Apps (ACA) with serverless GPU acceleration, use the following Azure CLI commands. This configuration targets an **NVIDIA T4 GPU** (`Consumption-GPU-NC8as-T4`).

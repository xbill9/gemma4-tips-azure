...
*   **Hardware:** NVIDIA RTX PRO 6000 GPU (1 unit) (or NVIDIA L4 GPU).
*   **Compute:** 20 vCPUs and 80GiB RAM.
*   **Execution Environment:** `gen2` (required for GPU and GCS FUSE).
*   **Storage:** A GCS Bucket containing the Gemma model weights (e.g., `gs://aisprint-491218-bucket/gemma-4-E4B-it/` or `gs://aisprint-491218-bucket/gemma-4-26B-A4B-it/`).
...
*   `GOOGLE_CLOUD_LOCATION`: The region for Vertex AI and Cloud Run (defaults to `us-central1`).
...
*   **`get_vllm_gpu_deployment_config`**: Generates a GKE manifest for GPU (NVIDIA L4/RTX 6000).
